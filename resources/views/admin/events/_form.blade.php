...
{!! Former::text('start_date')->class('form-control datepicker')->required()->label('Startdatum') !!}
{!! Former::checkbox('whole_day')->value('1')->text('Begintijd doet er niet toe')->label(null) !!}
{!! Former::text('start_time')->class('form-control timepicker')->label('Starttijd')->help('uu:mm') !!}
{!! Former::text('end_date')->class('form-control datepicker')->required()->label('Einddatum')->help('Als de activiteit maar een dag duurt, moet je dezelfde datum invullen als de startdatum') !!}
